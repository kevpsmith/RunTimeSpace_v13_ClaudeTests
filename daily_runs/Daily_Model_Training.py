...
class DailyModelTraining():
    def __init__(self, base_dir, date, device = "gpu"):
        base_dir = os.path.abspath(base_dir)
        self.base_dir = base_dir
        self.device = device
        self.analysis_date_end = date
        self.checkpoint_dir = os.path.join(self.base_dir, "my_model_checkpoints")
    ...
    def run(self):
        ...
        predictions_dir = os.path.join(self.base_dir, 'predictions_output_random')
        predictions_df.to_excel(f'{predictions_dir}/{analysis_date_end}_v12_NewData_RegimeAtt_DoubleDigit_random_random.xlsx', index=False)

if __name__ == "__main__":
    today = datetime.today().strftime("%Y-%m-%d")
    base_dir = "/teamspace/studios/this_studio"
    training = DailyModelTraining(base_dir, today)
    training.run()
